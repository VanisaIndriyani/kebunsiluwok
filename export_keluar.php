...
$filename = 'Laporan_Stok_Keluar_' . $selected_bulan;
...
if ($type == 'excel') {
    ...
    // Header Info
    $sheet->setCellValue('A1', 'LAPORAN STOK KELUAR');
    ...
    // Total Row
    $sheet->setCellValue('A' . $rowNum, 'TOTAL KELUAR');
    ...
    $sheet->getStyle('H7:H' . $lastRow)->getFont()->getColor()->setARGB('FFFF0000'); // Red for keluar
    ...
    <div style="text-align: center; margin-bottom: 20px;">
        <h3 style="margin: 0;">LAPORAN STOK KELUAR</h3>
    ...
    foreach ($transaksi as $row) {
        $html .= '
            <tr>
                ...
                <td style="text-align: right; font-weight: bold; color: red;">' . number_format($row['jumlah'], 2, ',', '.') . '</td>
            </tr>';
    }
    
    if (count($transaksi) == 0) {
        $html .= '<tr><td colspan="7" style="text-align: center;">Tidak ada data stok keluar.</td></tr>';
    }
